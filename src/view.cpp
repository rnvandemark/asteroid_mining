...
int AsteroidMiningViewer::get_time_rate() const
{
    return static_cast<int>(time_per_second);
}

bool AsteroidMiningViewer::showing_effective_forces() const
{
    return show_effective_forces;
}

double AsteroidMiningViewer::get_effective_force_markers_radius() const
{
    return effective_force_markers_radius;
}

bool AsteroidMiningViewer::corotating_camera_with_asteroid() const
{
    return corotate_camera_with_asteroid;
}

void AsteroidMiningViewer::change_time_rate_callback(const bool incremented)
{
...
void AsteroidMiningViewer::change_effective_force_markers_radius_callback(const bool incremented, const bool accelerated)
{
...
void AsteroidMiningViewer::post_draw()
{
    ...
    y += font_size * 1.5f * dpi_scaling();
    texter_->draw(
        "- angular position: " + std::to_string(math::round(model.get_siphon().get_siphon_angular_position() * 180 / M_PI, 3)) + " deg",
        x, y, font_size, 1, font_color
    );
    ...
    y += font_size * 1.5f * dpi_scaling();
    texter_->draw(
        "Last min / max siphon position: " + std::to_string(math::round(model.get_siphon().get_last_min_siphon_angular_position_reached() * 180 / M_PI, 3))
            + " / " + std::to_string(math::round(model.get_siphon().get_last_max_siphon_angular_position_reached() * 180 / M_PI, 3)) + " deg"
        ,
        x, y, font_size, 1, font_color
    );
    ...
    if (model.get_released_payload().is_active())
    {
        ...
        const auto stringify_v = [this] (const easy3d::vec3& v__frame) -> std::string
        {
            ...
            return "-- velocity: " + std::to_string(math::round(dimensions_scaler.get_dimensioned(
                ...
                + " m/s [" + std::to_string(math::round(v__frame.x / norm, 3))
                + ", " + std::to_string(math::round(v__frame.y / norm, 3))
                + ", " + std::to_string(math::round(v__frame.z / norm, 3)) + "]"
            ;
        };
...
View::DrawableMesh::DrawableMesh(
